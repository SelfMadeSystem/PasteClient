    ...
    public void render(float p_190060_1_, long p_190060_2_)
    {
        if (!this.pathMap.isEmpty())
        {
            ...
            for (Integer integer : this.pathMap.keySet())
            {
                ...
                {
                    RenderGlobal.renderFilledBox((new AxisAlignedBB((float)pathpoint.xCoord + 0.25F, (float)pathpoint.yCoord + 0.25F, (double)pathpoint.zCoord + 0.25D, (float)pathpoint.xCoord + 0.75F, (float)pathpoint.yCoord + 0.75F, (float)pathpoint.zCoord + 0.75F)).offset(-this.xo, -this.yo, -this.zo), 0.0F, 1.0F, 0.0F, 0.5F);
