    ...
    public void moveEntity(MoverType moverType, double x, double y, double z) {
        if (this.noClip) {
            ...
            if (moverType == MoverType.PISTON) {
                ...
                if (totalWorldTime != this.worldTime) {
                    Arrays.fill(this.field_191505_aI, 0.0D);
                    this.worldTime = totalWorldTime;
                }
            ...
            if ((moverType == MoverType.SELF || moverType == MoverType.PLAYER) && this.onGround && this.isSneaking() && this instanceof EntityPlayer) {
                for (double d5 = 0.05D; x != 0.0D && this.world.getCollisionBoxes(this, this.getEntityBoundingBox().offset(x, -this.stepHeight, 0.0D)).isEmpty(); x1 = x) {
                    if (x < 0.05D && x >= -0.05D) {
                        x = 0.0D;
                    } else if (x > 0.0D) {
                        x -= 0.05D;
                ...
                for (; z != 0.0D && this.world.getCollisionBoxes(this, this.getEntityBoundingBox().offset(0.0D, -this.stepHeight, z)).isEmpty(); z1 = z) {
                    if (z < 0.05D && z >= -0.05D) {
                        z = 0.0D;
                    } else if (z > 0.0D) {
                        z -= 0.05D;
                ...
                for (; x != 0.0D && z != 0.0D && this.world.getCollisionBoxes(this, this.getEntityBoundingBox().offset(x, -this.stepHeight, z)).isEmpty(); z1 = z) {
                    if (x < 0.05D && x >= -0.05D) {
                        x = 0.0D;
                    } else if (x > 0.0D) {
                        x -= 0.05D;
            ...
            boolean flag = this.onGround || y1 != y && y1 < 0.0D;

            if (this.stepHeight > 0.0F && flag && (x1 != x || z1 != z)) {
                StepEvent event = new StepEvent(this.stepHeight);
                EventManager.call(event);
                double d14 = x;
                double d6 = y;
                double d7 = z;
                AxisAlignedBB entityBoundingBox1 = this.getEntityBoundingBox();
                this.setEntityBoundingBox(entityBoundingBox);
                y = this.stepHeight;
                List<AxisAlignedBB> list = this.world.getCollisionBoxes(this, this.getEntityBoundingBox().addCoord(x1, y, z1));
                AxisAlignedBB entityBoundingBox2 = this.getEntityBoundingBox();
                AxisAlignedBB axisalignedbb3 = entityBoundingBox2.addCoord(x1, 0.0D, z1);
                double d8 = y;
                int j1 = 0;
                ...
                entityBoundingBox2 = entityBoundingBox2.offset(0.0D, d8, 0.0D);
                double d18 = x1;
                int l1 = 0;

                for (int i2 = list.size(); l1 < i2; ++l1) {
                    d18 = list.get(l1).calculateXOffset(entityBoundingBox2, d18);
                }

                entityBoundingBox2 = entityBoundingBox2.offset(d18, 0.0D, 0.0D);
                double d19 = z1;
                int j2 = 0;

                for (int k2 = list.size(); j2 < k2; ++j2) {
                    d19 = list.get(j2).calculateZOffset(entityBoundingBox2, d19);
                }

                entityBoundingBox2 = entityBoundingBox2.offset(0.0D, 0.0D, d19);
                ...
                if (d14 * d14 + d7 * d7 >= x * x + z * z) {
                    ...
                    y = d6;
                    z = d7;
                    this.setEntityBoundingBox(entityBoundingBox1);
                } else {
            ...
            this.resetPositionToBB();
            this.isCollidedHorizontally = x1 != x || z1 != z;
            this.isCollidedVertically = y1 != y;
            this.onGround = this.isCollidedVertically && y1 < 0.0D;
            ...
            if (this.canTriggerWalking() && (!this.onGround || !this.isSneaking() || !(this instanceof EntityPlayer)) && !this.isRiding()) {
                double d15 = this.posX - posX;
                double d16 = this.posY - posY;
                double d17 = this.posZ - posZ;
